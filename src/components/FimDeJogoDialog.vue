  ...
  <v-dialog v-model="dialog" max-width="400" persistent>
    <v-card class="text-center pa-6">
      <div class="d-flex flex-column align-center mb-2">
        ...
        <v-card-title class="justify-center pb-0">Fim de jogo!</v-card-title>
      </div>
      ...
      <v-card-text v-if="vencedor" class="pt-2">
        ...
        <p class="mb-0">venceu a partida!</p>
      </v-card-text>
      ...
      <v-card-actions class="d-flex flex-column mt-2">
        ...
        <p v-else class="caption mt-2 mb-0">
          Aguardando {{ nomeDono }} decidir se joga de novo...
        </p>

        <v-btn text class="mt-3" @click="$emit('sair')">Sair</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
